...
import type { AppSettings, EventItem, GalleryItem, GuestbookItem, WeddingConfig } from './types';
...
export const getInvitationData = cache(async (): Promise<InvitationData> => {
  try {
    const [settingsRows, weddingRows, eventRows, galleryRows, guestbookRows] = await Promise.all([
